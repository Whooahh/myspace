 ...
 * 		사원정보(사번, 이름, 급여, 상사명) 출력, 사원이 존재하지 않는 경우 (사원이 없습니다 출력)
 *  1.2 해당 부서번호가 존재하지 않는 경우 : 존재하지 않는 부서라고 출력 */
import java.sql.*;
...
public class Ex2_selectWhere {
	public static void main(String[] args) {
		...
		Statement stmt = null;
		ResultSet rs = null;
		System.out.print("출력을 원하는 부서번호를 입력하세요 : ");
		int deptno = scanner.nextInt();
		String query1 = "SELECT * FROM DEPT WHERE DEPTNO=" + deptno;
		String query2 = "SELECT W.EMPNO, W.ENAME, W.SAL, M.ENAME"
				+ "    FROM EMP W, EMP M"
		...
		} catch (SQLException e) {// 브서번호가 없는 경우
