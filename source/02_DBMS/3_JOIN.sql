...
SELECT * FROM EMP, DEPT WHERE ENAME='SMITH'; -- CROSS JOIN : 4행(1*4)
    ...
    -- 1. EQUI JOIN (동등 조인 = 공통필드 값이 일치되는 조건만 JOIN)
...
SELECT ENAME, JOB, SAL, E.DEPTNO FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND LOC='CHICAGO';
    -- EX. 82년도에 입사한 10번, 20번 부서 직원의 이름, 급여, 근무지 출력 (급여 순으로)
...
SELECT * FROM SALGRADE; -- 5행 (급여 등급 정보)
...
SELECT EMPNO, ENAME, MGR, SAL, GRADE||'등급'GRADE FROM EMP, SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL;
    -- 탄탄 EX1. COMM이 NULL이 아닌 사원의 이름, 급여, 급여등급, 부서번호, 부서이름, 근무지 출력
...
SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM, 0))*12 "ANNUALSAL", DNAME FROM EMP E, SALGRADE, DEPT D WHERE SAL BETWEEN LOSAL AND HISAL AND E.DEPTNO=D.DEPTNO ORDER BY DNAME, ANNUALSAL DESC; 
    -- 탄탄 EX4. 이름, 직책, 급여, 등급, 부서번호, 부서명(급여가 1000~3000사이, 정렬 조건 = 부서번호순, 직책순, 급여큰순)
SELECT ENAME, JOB, SAL, GRADE, E.DEPTNO, DNAME FROM EMP E, DEPT D, SALGRADE WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL AND SAL BETWEEN 1000 AND 3000 ORDER BY DEPTNO, JOB, SAL DESC;
    -- 탄탄 EX5. 이름, 급여, 등급, 입사일, 근무지(81년도 입사한 직원만 등급 큰 순으로 출력)
...
SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME FROM EMP W, EMP M WHERE W.MGR=M.EMPNO; --13행(KING의 MGR이 NULL이라서 EMPNO에 NULL이 없음)
DESC EMP;
    -- EX. 이름, 상사이름
SELECT W.ENAME, M.ENAME FROM EMP W, EMP M WHERE W.MGR=M.EMPNO;
    -- EX. SMITH의 상사는 FORD
SELECT W.ENAME || '의 상사는' || M.ENAME "MSG" FROM EMP W, EMP M WHERE W.MGR=M.EMPNO;    
    -- 탄탄 전 준비작업(20부서 : DALLAS)
INSERT INTO DEPT VALUES (50, 'IT', 'DALLAS');
INSERT INTO EMP VALUES (9999, '홍길동', NULL, NULL, NULL, 3000, NULL, 50);
SELECT * FROM DEPT;
SELECT * FROM EMP;
    -- 탄탄 EX1. 매니저가 KING인 사원들의 이름과 직급 출력
SELECT W.ENAME, W.JOB FROM EMP W, EMP M WHERE W.MGR=M.EMPNO AND M.ENAME ='KING';
    -- 탄탄 EX2. SCOTT과 동일한 부서번호에서 근무하는 사원의 이름 출력
SELECT E1.ENAME, E1.DEPTNO, E2.ENAME, E2.DEPTNO FROM EMP E1, EMP E2 WHERE E1.ENAME='SCOTT' AND E1.DEPTNO=E2.DEPTNO;
 ...
 SELECT E2.ENAME FROM EMP E1, EMP E2 WHERE E1.ENAME='SCOTT' AND E1.DEPTNO=E2.DEPTNO;
    -- 탄탄 EX3.SCOTT과 동일한 지역에서 근무하는 사원의 이름 출력
...
SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME FROM EMP W, EMP M WHERE W.MGR=M.EMPNO(+);
    -- SMITH의 상사는 FORD다(상사가 없는 직원은 KING의 상사는 없다고 출력)
SELECT W.ENAME || '의 상사는' || NVL(M.ENAME, '없') || '다' "MESSAGE" FROM EMP W, EMP M WHERE W.MGR=M.EMPNO(+);
SELECT W.ENAME, W.MGR, M.EMPNO, M.ENAME FROM EMP W, EMP M WHERE W.MGR=M.EMPNO(+);
    -- 말단 사원
